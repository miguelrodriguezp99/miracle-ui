...
const docsComponentsDir = path.resolve(
  rootDir,
  "apps/docs/content/docs/components"
); // Docs components directory path
...
function generateComponents() {
  ...
  const routes = routesJson.routes.find(
    (route) => route.key === "components"
  ).routes;
  ...
  for (const component of components) {
    ...
    const routeComponent =
      routes.find((route) => route.key === component) || {};

    // Add style alias for the component
    const mdxComponentPath = path.resolve(
      docsComponentsDir,
      `${component}.mdx`
    );
    const mdxComponentContent =
      (fs.existsSync(mdxComponentPath) &&
        fs.readFileSync(mdxComponentPath, "utf8")) ||
      "";
    ...
    const componentInfo = {
      ...
      status:
        (routeComponent.updated && "updated") ||
        (routeComponent.newPost && "new") ||
        "stable",
      style: style || "",
      peerDependencies:
        {
  ...
  fs.writeFileSync(
    path.resolve(outputPath, "components.json"),
